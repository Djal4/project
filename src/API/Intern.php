...
class Intern extends Response implements REST
{
    /**
     * User class instance
     * 
    ...
    public function post()
    {
        ...
        if( isset($data->name)    &&
            isset($data->lastname)&&
            isset($data->group_id)){
                if($this->usr->create($data->name,$data->lastname,1,$data->group_id)){
                    //Success message
    ...
    public function put()
    {
        ...
        if( isset($data['ID']) && !empty($this->usr->read($data['ID'])) && $this->usr->read($data['ID'])['role']=="Intern"){
    ...
    {
        //Checks is ID set and is there User registered as Intern
